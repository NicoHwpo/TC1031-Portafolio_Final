...
//O(n)
void Graph::ipSummary(){
  std::cout << "Introduzca ip: ";
  std::string ipstring;
  std::cin >> ipstring;
  getIpSummary(ipstring);
}


//O(V)
void Graph::buildHashTable(){
  //hash.setMaxSize(17387); // Colisiones 5210    1.3
  //hash.setMaxSize(21521);// Colisiones 4167     1.6
  hash.setMaxSize(32537);// Colisiones 3395       2.0
  //O(V)
  for(int i=1;i<=numNodes;i++){
    hash.add(ips[i].getIpValue(), ips[i]); //O(1)
  }
  
  std::cout << "Tamaño de HashTable: " << hash.getSize() << std::endl<< std::endl;    
  std::cout << "Numero de colisiones totales: " << hash.getColisiones()<<  std::endl<< std::endl;
}
...
//O(E + log V);
void Graph::getIpSummary(std::string ipString){
  int index = findIpIndex(ipString);//O(logv)
  int ipValue = ips[index].getIpValue();
  ipAddress IpData = hash.find(ipValue); //O(1)
  std::vector<ipAddress> ipsAdj;
  int tempIndex;
  ipAddress tempIp;
  LLNode<std::pair<int,int>> *ptr = adjList[index].getHead();
  if (IpData.getIpValue() != 0) {
    std::cout <<std::endl << "Ip address: "<<IpData.getIp()<<std::endl<<std::endl;
    std::cout << "Aristas saliendo: "<<IpData.getDegree()<<std::endl<<std::endl;
    std::cout << "Aristas entrando: "<<IpData.getDegreeIn()<<std::endl<<std::endl;

    //O(E)
    for (int i=1; i<= adjList[index].getNumElements(); i++) {
      tempIndex = ptr->data.first;
      tempIp = ips[tempIndex];
      ipsAdj.push_back(tempIp);
      ptr = ptr->next;
    }
    std::sort(ipsAdj.begin(), ipsAdj.end());
    std::cout << "La ip "<< ipString << " accesa a las siguientes ips:" << std::endl;
    int u=1;
    //O(E)
    for (std::vector<ipAddress>::iterator it=ipsAdj.begin(); it!=ipsAdj.end(); ++it){
      std::cout << "|\t  •" <<u<<") "<<it->getIp() << std::endl;
      u++;
    }
  }
  else
    std::cout << "Ip ingresada no encontrada." << std::endl;
}